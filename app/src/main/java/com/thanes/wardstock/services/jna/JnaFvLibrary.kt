...
abstract class FingerVeinLib {
  ...
  private var attachedDevCnt = 0
  private var detachedDevCnt = 0
  var onRawLog: ((log: String) -> Unit)? = null
  private val initDevice = mutableListOf<UsbDevice>()
  private val attachedDevice = mutableListOf<UsbDevice>()
  private val usbConnection = mutableListOf<UsbDeviceConnection>()
  protected var isInit = false

  abstract fun updateMsg(msg: String)

  companion object {
    private const val ACTION_USB_PERMISSION = "com.thanes.wardstock.USB_PERMISSION"
    private const val VID = 6353
    private const val PID = 48059
  }
          ...
          synchronized(this) {
            try {
              if (intent.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false)) {
                ...
                updateMsg("ผู้ใช้ปฏิเสธการอนุญาต")
              }
            } catch (e: Exception) {
              updateMsg("เกิดข้อผิดพลาดในการขออนุญาต USB: ${e.message}")
            }
          }
        }
        UsbManager.ACTION_USB_DEVICE_ATTACHED -> handleDeviceAttached(device)
  ...
  private fun handleDeviceAttached(device: UsbDevice?) {
    device?.let {
      if (VID == it.vendorId && PID == it.productId) {
        attachedDevCnt++
        updateMsg("พบอุปกรณ์ USB, กำลังขออนุญาต... ($attachedDevCnt)")
        attachedDevice.add(it)
        requestPermission(it)
      }
    }
  ...
  private fun handleDeviceDetached(device: UsbDevice?) {
    device?.let {
      if (VID == it.vendorId && PID == it.productId) {
        updateMsg("อุปกรณ์ USB ถูกถอดออก")
        detachedDevCnt++
        if (detachedDevCnt >= attachedDevCnt) {
          if (isInit) LibFvHelper.INSTANCE.fv_exit()
          closeConnection()
        }
      }
    }
  ...
  private fun requestPermission(device: UsbDevice) {
    try {
      ...
      updateMsg("เกิดข้อผิดพลาดในการขออนุญาต: ${e.message}")
    }
  }
  ...
  protected fun checkUser(uid: String): Boolean {
    if (uid.isBlank()) {
      updateMsg("ID ผู้ใช้ว่างเปล่า กรุณากรอก ID ผู้ใช้")
      return false
    }
    return true
  }
