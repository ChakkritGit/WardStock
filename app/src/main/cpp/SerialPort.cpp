...
Java_com_thanes_wardstock_services_usb_SerialPortJNI_openPort(
    ...
    struct termios options{};
    if (tcgetattr(fd, &options) != 0) {
        LOGE("tcgetattr failed for %s: %s", devicePath, strerror(errno));
        close(fd);
        env->ReleaseStringUTFChars(devicePath_, devicePath);
        return -1;
    }
    ...
    if (tcsetattr(fd, TCSANOW, &options) != 0) {
        LOGE("tcsetattr failed for %s: %s", devicePath, strerror(errno));
        close(fd);
        env->ReleaseStringUTFChars(devicePath_, devicePath);
        return -1;
    }
...
extern "C"
JNIEXPORT jint JNICALL
Java_com_thanes_wardstock_services_usb_SerialPortJNI_readBytes(
        JNIEnv *env,
        jobject,
        jint fd,
        jbyteArray buffer,
    ...
    ssize_t bytesRead = read(fd, buf, length);
    env->ReleaseByteArrayElements(buffer, buf, 0);

    if (bytesRead < 0 &&
        errno != EAGAIN) {
        LOGE("Read failed on fd=%d: %s", fd, strerror(errno));
        return -1;
    }
    return (jint) (bytesRead > 0 ? bytesRead : 0);
}

extern "C"
JNIEXPORT jint JNICALL
Java_com_thanes_wardstock_services_usb_SerialPortJNI_writeBytes(
        JNIEnv *env,
        jobject,
        jint fd,
        jbyteArray data) {
    ...
    if (bytesWritten < 0) {
        LOGE("Write failed on fd=%d: %s", fd, strerror(errno));
        return -1;
    }
...
extern "C"
JNIEXPORT void JNICALL
Java_com_thanes_wardstock_services_usb_SerialPortJNI_closePort(
        JNIEnv *env,
        jobject,
        jint fd) {
    if (fd >= 0) {
